...
Runtime and configuration Version changes automatically enqueue a domain-tag purge for Edge-enabled domains. `PurgeOnStartup` optionally purges an explicitly configured domain when a host starts. The built-in queue is in-memory and best-effort. Register an `IEdgeInvalidationQueue` first to route jobs to a durable outbox; an enqueue-only replacement also owns its durable dispatcher. Custom integrations implement `IEdgeResponseProvider` and `IEdgeInvalidationProvider`. Only tag-native providers are supported in v1.
