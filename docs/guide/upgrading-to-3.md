...
| Authentication | Opting into caching requires a stable user/tenant identity. A configured claim list defines the sharing boundary; a tenant claim alone permits sharing among that tenant's users. `Cache-Control: private` does not partition server caches. See [authenticated vary](../reference/vary.md). |
